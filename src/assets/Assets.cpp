...
std::string assetNameFromPath(const std::string& path)
{
    ...
    const size_t count = dot == std::string::npos || dot < start
                             ? std::string::npos
                             : dot - start;
...
SpriteDefinition spriteDefinitionFromJson(const json& spriteJSON,
                                          RenderBackend& renderBackend)
{
    ...
    const TextureHandle texture{spriteJSON.value("texture", name)};

    if (spriteJSON.contains("atlas"))
    {
        const auto& atlas = spriteJSON["atlas"];
        return SpriteDefinition(
            name, texture, frames, frametime, rows, cols,
            RectF{static_cast<float>(atlas["x"].get<int>()),
                  static_cast<float>(atlas["y"].get<int>()),
                  static_cast<float>(atlas["w"].get<int>()),
                  static_cast<float>(atlas["h"].get<int>())});
    }

    return SpriteDefinition(name, texture, frames, frametime, rows, cols,
...
void Assets::loadFromFile(const std::string& pathAssets,
                          RenderBackend& renderBackend, SDLPlatform& platform)
{
    ...
    for (const auto& spriteJSON : j["animations"])
    {
        SpriteDefinition sprite =
            spriteDefinitionFromJson(spriteJSON, renderBackend);
