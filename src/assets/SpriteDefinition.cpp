...
SpriteDefinition::SpriteDefinition(const std::string& name,
                                   TextureHandle texture, size_t frameCount,
                                   size_t frameDuration, int rows, int cols,
                                   TextureSize textureSize)
    : SpriteDefinition(name, texture, frameCount, frameDuration, rows, cols,
                       RectF{0.0f, 0.0f, static_cast<float>(textureSize.w),
...
SpriteDefinition::SpriteDefinition(const std::string& name,
                                   ...
                                   size_t frameDuration, int rows, int cols,
                                   RectF sourceRegion)
    : m_texture(texture), m_sourceRegion(sourceRegion), m_name(name),
      m_frameCount(std::max<size_t>(1, frameCount)),
      m_frameDuration(std::max<size_t>(1, frameDuration)),
      m_rows(std::max(1, rows)), m_cols(std::max(1, cols))
{
    m_frameSize = Vec2{m_sourceRegion.w / m_cols, m_sourceRegion.h / m_rows};
}
...
{
    return RectF{m_sourceRegion.x + col * m_frameSize.x,
                 m_sourceRegion.y + row * m_frameSize.y, m_frameSize.x,
                 m_frameSize.y};
}
