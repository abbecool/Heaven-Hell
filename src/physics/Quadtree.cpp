...
Quadtree::Quadtree(Vec2 pos, Vec2 size) : m_position(pos), m_size(size)
{
...
void Quadtree::subdivide()
{
    ...
    m_northWest = std::make_unique<Quadtree>(
        Vec2{x - subSize.x / 2, y - subSize.y / 2}, subSize);
...
void Quadtree::renderBoundary(RenderBackend& renderer, Color color)
{
    if (m_divided)
    {
    ...
    else
    {
        ...
        RectF collisionRect{x - width / 2, y - height / 2, width, height};
...
std::vector<std::shared_ptr<Quadtree>> Quadtree::createQuadtreeVector()
{
    ...
    if (m_divided)
    {
        ...
        quadtreeVector.insert(quadtreeVector.end(), nwVector.begin(),
                              nwVector.end());
        quadtreeVector.insert(quadtreeVector.end(), neVector.begin(),
                              neVector.end());
        quadtreeVector.insert(quadtreeVector.end(), swVector.begin(),
                              swVector.end());
        quadtreeVector.insert(quadtreeVector.end(), seVector.begin(),
                              seVector.end());
    }
    else
    {
        quadtreeVector.push_back(
            std::shared_ptr<Quadtree>(this, [](Quadtree*) {}));
    }
