...
std::vector<TileRect> mergeTileRects(const std::vector<bool>& occupied,
...
void appendMergedColliderShapes(std::vector<ColliderShape>& shapes,
                                const std::vector<bool>& occupied, int width,
                                int height, const Vec2& tileSize,
                                CollisionMask layer, CollisionMask targetMask,
                                Color debugColor, bool isTrigger)
{
    for (const TileRect& rect : mergeTileRects(occupied, width, height))
    {
        const Vec2 size{static_cast<float>(rect.width) * tileSize.x,
                        static_cast<float>(rect.height) * tileSize.y};
        const Vec2 offset{(static_cast<float>(rect.x) +
                           static_cast<float>(rect.width) / 2.0f) *
                              tileSize.x,
                          (static_cast<float>(rect.y) +
                           static_cast<float>(rect.height) / 2.0f) *
                              tileSize.y};
        shapes.emplace_back(offset, size, layer, targetMask, debugColor,
                            isTrigger);
    }
}
...
LevelLoader::LevelLoader(Scene* scene, const Vec2 gridSize,
                         const PixelImage& levelImage, bool buildColliders)
{
    m_scene = scene;
    m_gridSize = gridSize;
    m_buildColliders = buildColliders;
    if (levelImage.width <= 0 || levelImage.height <= 0 ||
        levelImage.pixels.empty())
    {
    ...
    m_levelSize =
        Vec2{static_cast<float>(m_width), static_cast<float>(m_height)};
    createPixelMatrix(levelImage);
    loadChunk(m_currentChunk);
}

std::array<bool, 4> LevelLoader::neighborCheck(int x, int y, int width,
                                               int height)
{
    ...
    if (!neighbors[0])
    {
        neighbors[0] = (y > 0 && m_pixelMatrix[(y - 1) * width + x] == pix);
    } // top
    if (!neighbors[1])
    {
        neighbors[1] =
            (x < width - 1 && m_pixelMatrix[y * width + (x + 1)] == pix);
    } // right
    if (!neighbors[2])
    {
        neighbors[2] =
            (y < height - 1 && m_pixelMatrix[(y + 1) * width + x] == pix);
    } // bottom
    if (!neighbors[3])
    {
        neighbors[3] = (x > 0 && m_pixelMatrix[y * width + (x - 1)] == pix);
    } // left
    return neighbors;
}

std::array<TileType, 4> LevelLoader::neighborTag(int x, int y, int width,
                                                 int height)
{
    ...
    if (y > 0)
    {
        neighborsTags[0] = m_pixelMatrix[(y - 1) * width + x];
    } // top
    if (x < width - 1)
    {
        neighborsTags[1] = m_pixelMatrix[y * width + (x + 1)];
    } // right
    if (y < height - 1)
    {
        neighborsTags[2] = m_pixelMatrix[(y + 1) * width + x];
    } // bottom
    if (x > 0)
    {
        neighborsTags[3] = m_pixelMatrix[y * width + (x - 1)];
    } // left
...
{
    int numObstacles =
        static_cast<int>(std::count(neighbors.begin(), neighbors.end(), true));
    if (numObstacles == 1)
    {
        if (neighbors[0])
            return 12; // Top
    ...
    else if (numObstacles == 2)
    {
        ...
        if (!neighbors[1] && !neighbors[2])
            return 15; // Right & Bottom
        if (!neighbors[2] && !neighbors[3])
            return 13; // Bottom & Left
    ...
    {
        if (!neighbors[0])
            return 6; // Top is not an obstacle
        if (!neighbors[1])
            return 11; // Right is not an obstacle
        if (!neighbors[2])
            return 14; // Bottom is not an obstacle
        if (!neighbors[3])
            return 9; // Left is not an obstacle
    }
...
void LevelLoader::createPixelMatrix(const PixelImage& levelImage)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            if (static_cast<int>(pixel.r) == 192 &&
                static_cast<int>(pixel.g) == 192 &&
                static_cast<int>(pixel.b) == 192)
            {
                tile = TileType::OBSTACLE;
            }
            else if (static_cast<int>(pixel.r) == 203 &&
                     static_cast<int>(pixel.g) == 129 &&
                     static_cast<int>(pixel.b) == 56)
            {
                tile = TileType::DIRT;
            }
            else if (static_cast<int>(pixel.r) == 0 &&
                     static_cast<int>(pixel.g) == 255 &&
                     static_cast<int>(pixel.b) == 0)
            {
                tile = TileType::GRASS;
            }
            else if (static_cast<int>(pixel.r) == 0 &&
                     static_cast<int>(pixel.g) == 0 &&
                     static_cast<int>(pixel.b) == 255)
            {
                tile = TileType::WATER;
            }
            else if (static_cast<int>(pixel.r) == 179 &&
                     static_cast<int>(pixel.g) == 0 &&
                     static_cast<int>(pixel.b) == 255)
            {
...
std::array<int, 5> LevelLoader::createDualGrid(int x, int y)
{
    ...
    tileQ[0] = (x > 0) ? m_pixelMatrix[y * m_width + (x - 1)]
                       : m_pixelMatrix[y * m_width + x]; // Q3
    tileQ[1] = m_pixelMatrix[y * m_width + x];           // Q4
    tileQ[2] = (y > 0) ? m_pixelMatrix[(y - 1) * m_width + x]
                       : m_pixelMatrix[y * m_width + x]; // Q1
    tileQ[3] = (x > 0 && y > 0) ? m_pixelMatrix[(y - 1) * m_width + (x - 1)]
                                : m_pixelMatrix[y * m_width + x]; // Q2

    for (TileType tile : tileQ)
    {

        int numTiles =
            static_cast<int>(std::count(tileQ.begin(), tileQ.end(), tile));
        int uniqueCount = 0;
        ...
        {
            if (tileQ[0] != tile && tileQ[1] != tile)
                textureIndex = 13;
            if (tileQ[1] != tile && tileQ[2] != tile)
                textureIndex = 15;
            if (tileQ[2] != tile && tileQ[3] != tile)
                textureIndex = 7;
            if (tileQ[3] != tile && tileQ[0] != tile)
                textureIndex = 5;
            if (tileQ[0] != tile && tileQ[2] != tile)
                textureIndex = 8;
            if (tileQ[1] != tile && tileQ[3] != tile)
                textureIndex = 2;
        ...
        else if (numTiles == 1)
        {
            ...
            if (uniqueCount == 3 && (tile == TileType::GRASS))
            {
                if (tileQ[0] == tile && tileQ[2] == tileQ[3])
                    textureIndex = 21;
                if (tileQ[0] == tile && tileQ[1] == tileQ[2])
                    textureIndex = 23;

                if (tileQ[1] == tile && tileQ[0] == tileQ[3])
                    textureIndex = 20;
                if (tileQ[1] == tile && tileQ[2] == tileQ[3])
                    textureIndex = 22;

                if (tileQ[2] == tile && tileQ[0] == tileQ[1])
                    textureIndex = 24;
                if (tileQ[2] == tile && tileQ[0] == tileQ[3])
                    textureIndex = 26;

                if (tileQ[3] == tile && tileQ[1] == tileQ[2])
                    textureIndex = 25;
                if (tileQ[3] == tile && tileQ[0] == tileQ[1])
                    textureIndex = 27;
            }
        }
...
EntityID LevelLoader::loadChunk(Vec2 chunk)
{
    ...
    const int chunkEndX =
        std::min(chunkStartX + static_cast<int>(m_chunkSize.x), m_width);
    const int chunkEndY =
        std::min(chunkStartY + static_cast<int>(m_chunkSize.y), m_height);
    const int chunkWidth = std::max(0, chunkEndX - chunkStartX);
    const int chunkHeight = std::max(0, chunkEndY - chunkStartY);
    const Vec2 chunkOrigin{static_cast<float>(chunkStartX) * m_gridSize.x,
    ...
    std::vector<bool> obstacleTiles(
        static_cast<size_t>(chunkWidth * chunkHeight), false);
    std::vector<bool> waterTiles(static_cast<size_t>(chunkWidth * chunkHeight),
                                 false);
    ...
    for (int y = chunkStartY; y < chunkEndY; ++y)
    {
        for (int x = chunkStartX; x < chunkEndX; ++x)
        {
            ...
            std::vector<EntityID> ids = m_scene->spawnDualTiles(
                Vec2{16.0f * static_cast<float>(x) - 8.0f,
                     16.0f * static_cast<float>(y) - 8.0f},
                tileIndex);
            for (EntityID id : ids)
            {
                m_scene->m_ECS.addComponent<CStatic>(id);
                const Vec2 relativePos =
                    m_scene->m_ECS.getComponent<CTransform>(id).pos -
                    chunkOrigin;
            ...
            const size_t localIndex =
                static_cast<size_t>(localY * chunkWidth + localX);
    ...
    if (m_buildColliders)
    {
        ...
        const CollisionMask obstacleMask =
            ENEMY_LAYER | FRIENDLY_LAYER | PLAYER_LAYER | PROJECTILE_LAYER;
        appendMergedColliderShapes(
            colliderShapes, obstacleTiles, chunkWidth, chunkHeight, m_gridSize,
            OBSTACLE_LAYER, obstacleMask, Color{255, 255, 255, 255}, false);
        const CollisionMask waterMask =
            ENEMY_LAYER | FRIENDLY_LAYER | PLAYER_LAYER;
        appendMergedColliderShapes(colliderShapes, waterTiles, chunkWidth,
                                   chunkHeight, m_gridSize, WATER_LAYER,
                                   waterMask, Color{0, 0, 255, 255}, true);
        if (!colliderShapes.empty())
        {
            m_scene->m_ECS.addComponent<CCollider>(chunkID,
                                                   std::move(colliderShapes));
        }
    }
...
void LevelLoader::renderChunkGrid(RenderBackend& renderer) const
{
    ...
    for (auto [id, chunk, transform] :
         m_scene->m_ECS.constView<CChunk, CTransform>())
    {
        const int chunkStartX =
            static_cast<int>(transform.pos.x / m_gridSize.x);
        const int chunkStartY =
            static_cast<int>(transform.pos.y / m_gridSize.y);
        const int chunkEndX =
            std::min(chunkStartX + static_cast<int>(m_chunkSize.x), m_width);
        const int chunkEndY =
            std::min(chunkStartY + static_cast<int>(m_chunkSize.y), m_height);
        ...
        renderer.drawWorldRect(
            RectF{transform.pos.x, transform.pos.y,
...
void LevelLoader::update(Vec2 playerPosition)
{
    ...
    m_currentChunk =
        (((playerPosition / m_gridSize).toInt()) / m_chunkSize).toInt();
    ...
    for (int dx = -2; dx <= 2; ++dx)
    {
        for (int dy = -1; dy <= 1; ++dy)
        {
            ...
            if (neighborChunk.smaller(Vec2{0, 0}) ||
                neighborChunk.greater(m_levelSize / m_chunkSize))
            {
                continue;
            }
            m_neighboringChunks.push_back(neighborChunk);
            if (std::find(m_loadedChunks.begin(), m_loadedChunks.end(),
                          neighborChunk) == m_loadedChunks.end() &&
                std::find(m_chunkQueue.begin(), m_chunkQueue.end(),
                          neighborChunk) == m_chunkQueue.end())
            {
    ...
    {
        if (std::find(m_neighboringChunks.begin(), m_neighboringChunks.end(),
                      chunk) == m_neighboringChunks.end())
        {
