...
public:
    ...
    void insert(size_t objectIndex, Vec2 objectCenter, Vec2 objectSize)
    {
        ...
        if (!intersects(objectCenter, objectSize, m_position, m_size))
        {
            return;
        }
        if (m_divided)
        ...
        m_objects.push_back(
            ObjectBounds{objectIndex, objectCenter, objectSize});
    ...
    static bool intersects(Vec2 objectCenter, Vec2 objectSize, Vec2 treeCenter,
                           Vec2 treeSize)
    {
        ...
        bool x_overlap =
            (aPos.x + aSize.x > bPos.x) && (bPos.x + bSize.x > aPos.x);
        bool y_overlap =
