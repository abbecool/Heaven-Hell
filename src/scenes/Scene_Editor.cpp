...
const char* categoryLabel(EditorEntityCategory category)
{
    switch (category)
    {
    case EditorEntityCategory::Character:
        return "Characters";
    case EditorEntityCategory::Item:
        return "Items";
    case EditorEntityCategory::Structure:
        return "Structures";
    }
    return "Entities";
}
...
Scene_Editor::Scene_Editor(Game* game) : Scene(game)
{
...
Vec2 Scene_Editor::scenePlayStartPosition() const
{
    ...
    const std::string spriteName = player.at("components")
                                       .at("CAnimation")
                                       .at("animation")
                                       .get<std::string>();
...
void Scene_Editor::drawText(const std::string& text, RectF rect,
                            Color color) const
{
    m_game->render().drawText(
        TextDrawCommand{text, "Minecraft", screenRect(rect), color});
}

RectF Scene_Editor::screenRect(const RectF& virtualRect) const
{
    const float scale = static_cast<float>(m_game->getScale());
    return {virtualRect.x * scale, virtualRect.y * scale, virtualRect.w * scale,
            virtualRect.h * scale};
}

void Scene_Editor::drawButton(const RectF& rect, const std::string& label,
                              bool active) const
{
    m_game->render().fillRect(screenRect(rect),
                              active ? ActiveColor : ButtonColor);
    m_game->render().drawRect(screenRect(rect), {190, 210, 235, 255});
    drawText(label,
             {rect.x + 3.0f, rect.y + 2.0f, rect.w - 6.0f, rect.h - 4.0f});
}

void Scene_Editor::sRenderPicker()
{
    m_game->render().fillRect(
        screenRect({0.0f, 0.0f, static_cast<float>(width()),
                    static_cast<float>(height())}),
        PanelColor);
    drawText("LAYOUT EDITOR", {24.0f, 16.0f, 250.0f, 24.0f},
             {255, 235, 160, 255});
    drawButton({24.0f, 46.0f, 110.0f, 22.0f}, "NEW LAYOUT");
    drawText("Open a layout, or mark one as the layout Scene_Play uses.",
             {150.0f, 49.0f, 440.0f, 18.0f});
    ...
    for (const LayoutInfo& info : m_layouts.layouts())
    {
        ...
        m_game->render().fillRect(screenRect({18.0f, y, 604.0f, 26.0f}),
                                  active ? Color{37, 70, 90, 255}
                                         : Color{31, 42, 58, 255});
        drawText((active ? "* " : "") + info.displayName,
                 {24.0f, y + 4.0f, 190.0f, 18.0f});
        drawText(active ? "ACTIVE IN PLAY" : info.id,
                 {214.0f, y + 4.0f, 120.0f, 18.0f},
                 active ? Color{155, 230, 165, 255}
    ...
    drawText(m_status,
             {24.0f, static_cast<float>(height()) - 25.0f, 590.0f, 18.0f},
             {255, 235, 160, 255});
}
...
void Scene_Editor::sRenderWorkspace()
{
    ...
    if (m_selectedEntity && m_ECS.isAlive(*m_selectedEntity) &&
        m_ECS.hasComponent<CTransform>(*m_selectedEntity) &&
        m_ECS.hasComponent<CSprite>(*m_selectedEntity))
    {
        const CTransform& transform =
            m_ECS.getComponent<CTransform>(*m_selectedEntity);
        const CSprite& sprite = m_ECS.getComponent<CSprite>(*m_selectedEntity);
        const Vec2 size = sprite.size() * transform.scale;
        m_game->render().drawWorldRect({transform.pos.x - size.x / 2.0f - 2.0f,
                                        transform.pos.y - size.y / 2.0f - 2.0f,
                                        size.x + 4.0f, size.y + 4.0f},
                                       {255, 220, 70, 255});
    }

    m_game->render().fillRect(
        screenRect({0.0f, 0.0f, static_cast<float>(width()), ToolbarHeight}),
        PanelColor);
    drawButton({8.0f, 7.0f, 68.0f, 20.0f}, "SELECT",
               m_mode == EditMode::Select);
    drawButton({80.0f, 7.0f, 62.0f, 20.0f}, "PLACE", m_mode == EditMode::Place);
    drawButton({146.0f, 7.0f, 66.0f, 20.0f}, "DELETE",
               m_mode == EditMode::Delete);
    drawButton({220.0f, 7.0f, 52.0f, 20.0f}, "SAVE");
    drawButton({276.0f, 7.0f, 66.0f, 20.0f}, "EXIT");
    drawText(m_currentLayout.displayName + (m_dirty ? " *" : ""),
             {355.0f, 9.0f, 270.0f, 18.0f}, {255, 235, 160, 255});
    ...
    drawButton(characterTab, "CHARACTERS",
               m_selectedCategory == EditorEntityCategory::Character);
    drawButton(itemTab, "ITEMS",
               m_selectedCategory == EditorEntityCategory::Item);
    drawButton(structureTab, "STRUCTURES",
               m_selectedCategory == EditorEntityCategory::Structure);
    drawText("Mouse wheel over this panel changes palette page.",
             {300.0f, 37.0f, 310.0f, 16.0f}, {190, 200, 215, 255});

    const std::vector<EditorEntityDefinition>& entries =
        m_catalog.category(m_selectedCategory);
    const int visibleCount = 7;
    const int maxOffset =
        std::max(0, static_cast<int>(entries.size()) - visibleCount);
    const int offset = std::clamp(m_paletteOffset, 0, maxOffset);
    for (int i = 0;
         i < visibleCount && offset + i < static_cast<int>(entries.size()); ++i)
    {
        const EditorEntityDefinition& definition = entries[offset + i];
        const RectF rect{8.0f + static_cast<float>(i) * 90.0f, 60.0f, 86.0f,
                         20.0f};
        drawButton(rect, definition.displayName,
                   definition.id == m_selectedDefinition);
    }
    drawText("Mode: " +
                 std::string(m_mode == EditMode::Select  ? "Select"
                             : m_mode == EditMode::Place ? "Place"
                                                         : "Delete") +
                 " | Palette: " + categoryLabel(m_selectedCategory) +
                 " | U saves, Esc exits",
             {8.0f, 87.0f, 615.0f, 18.0f});
    drawText(m_status,
             {8.0f, static_cast<float>(height()) - 22.0f, 620.0f, 16.0f},
             {255, 235, 160, 255});
}
...
void Scene_Editor::sRenderConfirmation()
{
    m_game->render().fillRect(
        screenRect({0.0f, 0.0f, static_cast<float>(width()),
                    static_cast<float>(height())}),
        {0, 0, 0, 170});
    m_game->render().fillRect(screenRect({130.0f, 130.0f, 380.0f, 94.0f}),
                              PanelColor);
    m_game->render().drawRect(screenRect({130.0f, 130.0f, 380.0f, 94.0f}),
                              {255, 235, 160, 255});
    drawText("Save changes before leaving the editor?",
             {150.0f, 148.0f, 340.0f, 20.0f});
...
EntityID Scene_Editor::spawnPreview(const LayoutPlacement& placement)
{
    const EditorEntityDefinition* definition =
        m_catalog.find(placement.definition);
    if (!definition)
    {
    ...
    m_ECS.addComponent<CEditorPlacement>(entity, placement.definition,
                                         placement.x, placement.y);
    addVisual(entity, definition->spriteName, definition->renderLayer, true);
    const Vec2 pixelPosition{static_cast<float>(placement.x) * m_gridSize.x,
                             static_cast<float>(placement.y) * m_gridSize.y};
    m_ECS.addComponent<CTransform>(entity,
                                   gridToMidPixel(pixelPosition, entity));
    spawnPreviewShadow(entity, *definition);
    return entity;
}

EntityID
Scene_Editor::spawnPreviewShadow(EntityID parentID,
                                 const EditorEntityDefinition& definition)
{
    ...
    const float shadowScale =
        parentSize.x / shadowSprite.frameSize().x * definition.shadowScale;
    if (shadowScale == 0.0f)
    {
        return static_cast<EntityID>(-1);
    }
    ...
    const Vec2 relativePosition{definition.shadowOffset.x,
                                parentSize.y / 2.0f - shadowSize.y / 2.0f +
                                    definition.shadowOffset.y};
...
void Scene_Editor::openLayout(const std::string& id)
{
    ...
    try
    {
        ...
        {
            throw std::runtime_error("Could not load terrain image: " +
                                     info.terrainPath);
        }
        ...
        m_camera.calibrate(
            ...
            m_levelLoader.getLevelSize(), m_gridSize);
        m_camera.position =
            scenePlayStartPosition() - m_camera.getScreenSize() / 2.0f;
        ...
        const float visibleWidth =
            static_cast<float>(m_game->getWidth()) / viewScale;
        const float visibleHeight =
            static_cast<float>(m_game->getHeight()) / viewScale;
        m_camera.position.x =
            std::clamp(m_camera.position.x, 0.0f,
                       std::max(0.0f, worldSize.x - visibleWidth));
        m_camera.position.y =
            std::clamp(m_camera.position.y, 0.0f,
        ...
        m_status = m_unresolvedPlacements.empty()
                       ...
                       : "Some unknown or out-of-bounds placements are "
...
void Scene_Editor::duplicateLayout(const std::string& id)
{
    ...
    {
        m_status =
            std::string("Could not duplicate layout: ") + exception.what();
    }
}
...
void Scene_Editor::setActiveLayout(const std::string& id)
{
    try
    {
        ...
        m_status =
            m_layouts.layout(id).displayName + " is now active in Scene_Play.";
    }
    catch (const std::exception& exception)
    {
        m_status =
            std::string("Could not activate layout: ") + exception.what();
    }
}
...
WorldLayout Scene_Editor::buildLayout() const
{
    ...
    for (auto [entity, placement, transform] :
         m_ECS.constView<CEditorPlacement, CTransform>())
    {
        layout.placements.push_back(
            LayoutPlacement{placement.definition, placement.x, placement.y});
    }
    std::sort(layout.placements.begin(), layout.placements.end(),
              [](const LayoutPlacement& lhs, const LayoutPlacement& rhs)
              {
...
Vec2 Scene_Editor::screenToWorld(Vec2 screenPoint)
{
    ...
    return {(screenPoint.x * windowScale - view.originX) / view.scale +
                view.cameraX,
            (screenPoint.y * windowScale - view.originY) / view.scale +
                view.cameraY};
}

Vec2 Scene_Editor::snappedGrid(Vec2 worldPoint) const
{
    return {std::floor(worldPoint.x / m_gridSize.x),
...
std::optional<EntityID> Scene_Editor::previewAt(Vec2 worldPoint) const
{
    ...
    int topLayer = -1;
    for (auto [entity, placement, transform, sprite] :
         m_ECS.constView<CEditorPlacement, CTransform, CSprite>())
    {
        const Vec2 size = sprite.size() * transform.scale;
        const RectF bounds{transform.pos.x - size.x / 2.0f,
                           transform.pos.y - size.y / 2.0f, size.x, size.y};
...
void Scene_Editor::placeAt(Vec2 worldPoint)
{
    ...
    const EntityID entity = spawnPreview(
        LayoutPlacement{m_selectedDefinition, static_cast<int>(grid.x),
...
void Scene_Editor::updateCamera()
{
    ...
    const float visibleHeight =
        static_cast<float>(m_game->getHeight()) / viewScale;
    m_camera.position.x = std::clamp(
        m_camera.position.x, 0.0f, std::max(0.0f, worldSize.x - visibleWidth));
    m_camera.position.y = std::clamp(
        m_camera.position.y, 0.0f, std::max(0.0f, worldSize.y - visibleHeight));
}

void Scene_Editor::updateDrag()
{
    if (!m_leftHeld || !m_dragging || !m_selectedEntity ||
        !m_ECS.isAlive(*m_selectedEntity))
    {
        return;
    }
    ...
    if (transform.pos != target)
    {
        ...
        CEditorPlacement& placement =
            m_ECS.getComponent<CEditorPlacement>(*m_selectedEntity);
...
void Scene_Editor::handleWorkspaceClick(Vec2 point)
{
    ...
    {
        if (contains({8.0f, 7.0f, 68.0f, 20.0f}, point))
            m_mode = EditMode::Select;
        else if (contains({80.0f, 7.0f, 62.0f, 20.0f}, point))
            m_mode = EditMode::Place;
        else if (contains({146.0f, 7.0f, 66.0f, 20.0f}, point))
            m_mode = EditMode::Delete;
        else if (contains({220.0f, 7.0f, 52.0f, 20.0f}, point))
            saveCurrentLayout();
        else if (contains({276.0f, 7.0f, 66.0f, 20.0f}, point))
            requestExit();
        ...
        else if (point.y >= 60.0f && point.y <= 80.0f)
        {
            ...
            if (index >= 0 && index < 7 && selectedIndex >= 0 &&
                selectedIndex < static_cast<int>(entries.size()))
            {
    ...
    else
    {
        ...
        m_status = m_selectedEntity ? "Drag the selected entity to move it."
                                    : "Nothing selected.";
    }
}
...
void Scene_Editor::sDoAction(const Action& action)
{
    if (action.type() == "START")
    {
        if (action.name() == "PRIMARY")
        {
            m_leftHeld = true;
            if (m_confirmExit)
                handleWorkspaceClick(m_mousePosition);
            else if (m_view == EditorView::LayoutPicker)
                handlePickerClick(m_mousePosition);
            else
                handleWorkspaceClick(m_mousePosition);
        }
        else if (action.name() == "SECONDARY" &&
                 m_view == EditorView::Workspace)
            handleWorkspaceRightClick(m_mousePosition);
            ...
            requestExit();
        else if (action.name() == "DELETE" && m_selectedEntity &&
                 m_ECS.isAlive(*m_selectedEntity))
        {
    ...
    else if (action.name() == "PALETTE_SCROLL" &&
             m_view == EditorView::Workspace &&
             m_mousePosition.y < ToolbarHeight)
    {
        const int size =
            static_cast<int>(m_catalog.category(m_selectedCategory).size());
        m_paletteOffset = std::clamp(m_paletteOffset - getMouseState().scroll,
                                     0, std::max(0, size - 7));
    }
}
...
void Scene_Editor::update()
{
    if (m_view == EditorView::Workspace)
    {
        ...
        const Vec2 focus =
            m_camera.position +
            Vec2{static_cast<float>(m_game->getWidth()) / (2.0f * viewScale),
