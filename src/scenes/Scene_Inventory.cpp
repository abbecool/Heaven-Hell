...
Scene_Inventory::Scene_Inventory(Game* game) : Scene(game)
{
...
void Scene_Inventory::sRender()
{
    if (m_open)
    {
        ...
        drawSprite(inventorySprite, RectF{pos.x, pos.y, inventorySprite.src.w,
                                          inventorySprite.src.h});
    }
    CSprite hotbar(getSprite("inventory_open"), 0);
    hotbar.src.w = (Vec2{4, 1} * 8).x;
    hotbar.src.h = (Vec2{4, 1} * 8).y;
    Vec2 hotbarPos =
        Vec2{m_game->getWidth() - hotbar.src.w, 0.0f} + Vec2{-4, 4};
    drawSprite(hotbar,
               RectF{hotbarPos.x, hotbarPos.y, hotbar.src.w, hotbar.src.h});
    ...
    for (auto [eID, transform, sprite] : m_ECS.constView<CTransform, CSprite>())
    {
        ...
        drawSprite(sprite,
                   RectF{adjustedPos.x - destSize.x / 2,
                         adjustedPos.y - destSize.y / 2, destSize.x,
                         destSize.y},
                   transform.angle);
    }
}
...
void Scene_Inventory::spawnItem(std::string sprite)
{
    ...
    Vec2 pos = {static_cast<float>(itemIndex % columnCount),
...
void Scene_Inventory::Scroll(int scroll)
{
    ...
    const int columnCount = static_cast<int>(m_inventorySize.x);
    const int column =
        static_cast<int>(pos.x + static_cast<float>(scroll)) % columnCount;
