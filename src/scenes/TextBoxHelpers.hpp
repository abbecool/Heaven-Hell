...
inline Vec2 measureTextBox(const CText& text, const CSprite& sprite,
                           float padding)
{
    const Vec2 spriteSize = sprite.size();

    return Vec2{std::max(spriteSize.x, text.size.x + padding * 2.0f),
                std::max(spriteSize.y, text.size.y + padding * 2.0f)};
}

inline void configureSpriteBackedTextBox(CText& text, CTransform& transform,
                                         const CSprite& sprite, float padding)
{
    ...
    transform.scale = Vec2{boxSize.x / spriteSize.x, boxSize.y / spriteSize.y};
}
