...
Scene_Pause::Scene_Pause(Game* game) : Scene(game)
{
...
{
    json j;
    for (auto [e, dialog, collider, transform] :
         m_ECS.constView<CText, CCollider, CTransform>())
    {
        json button;
    ...
    if (!file)
    {
        throw std::runtime_error("Could not open button placement file: " +
                                 filename);
    }
    file << j.dump(4);
    file.close();
    ...
    if (action.type() == "END")
    {
        if (action.name() == "CLICK")
        {
            ...
            for (auto [e, collider, transform, nameComponent] :
                 m_ECS.constView<CCollider, CTransform, CName>())
            {
                ...
                if (name == "CONTINUE")
                {
                    if (m_game->sceneMap().find("PLAY") !=
                        m_game->sceneMap().end())
                    {
                        auto playScene = std::dynamic_pointer_cast<Scene_Play>(
                            m_game->sceneMap().at("PLAY"));
...
void Scene_Pause::sDragButton()
{
    ...
    {
        for (auto [e, collider, transform, name] :
