...
void requireString(const json& value, const char* field,
                   const std::filesystem::path& source)
{
    if (!value.contains(field) || !value.at(field).is_string() ||
        value.at(field).get<std::string>().empty())
    {
        throw std::runtime_error("Missing or invalid '" + std::string(field) +
                                 "' in " + source.string());
    }
}
...
LayoutRepository::LayoutRepository(std::filesystem::path registryPath)
    : m_registryPath(std::move(registryPath)),
      m_mirrorToSource(m_registryPath ==
                       std::filesystem::path("config_files/levels.json"))
{
}
...
void LayoutRepository::load()
{
    ...
    {
        throw std::runtime_error("Unsupported level registry: " +
                                 m_registryPath.string());
    }
    requireString(registry, "activeLayoutId", m_registryPath);
    if (!registry.contains("layouts") || !registry.at("layouts").is_array() ||
        registry.at("layouts").empty())
    {
        throw std::runtime_error(
            "Level registry must contain at least one layout: " +
            m_registryPath.string());
    }
    ...
    for (const json& entry : registry.at("layouts"))
    {
        if (!entry.is_object())
        {
            throw std::runtime_error("Invalid layout entry in " +
                                     m_registryPath.string());
        }
        ...
        LayoutInfo info{entry.at("id").get<std::string>(),
        ...
        {
            throw std::runtime_error("Duplicate layout id in " +
                                     m_registryPath.string() + ": " + info.id);
        }
        layouts.push_back(std::move(info));
    }

    const std::string activeLayoutId =
        registry.at("activeLayoutId").get<std::string>();
    if (std::none_of(layouts.begin(), layouts.end(),
                     [&activeLayoutId](const LayoutInfo& info)
                     { return info.id == activeLayoutId; }))
    {
        throw std::runtime_error("Active layout does not exist: " +
                                 activeLayoutId);
    }
...
const std::vector<LayoutInfo>& LayoutRepository::layouts() const
{
    if (!m_loaded)
    {
        throw std::logic_error(
            "LayoutRepository::load() must be called before layouts()");
    }
    return m_layouts;
}
...
const LayoutInfo* LayoutRepository::find(const std::string& id) const
{
    const auto it =
        std::find_if(m_layouts.begin(), m_layouts.end(),
                     [&id](const LayoutInfo& info) { return info.id == id; });
...
const LayoutInfo& LayoutRepository::layout(const std::string& id) const
{
    ...
    {
        throw std::logic_error(
            "LayoutRepository::load() must be called before layout()");
    }
...
WorldLayout LayoutRepository::loadLayout(const LayoutInfo& info) const
{
    ...
    {
        if (!entry.is_object() || !entry.contains("definition") ||
            !entry.at("definition").is_string() || !entry.contains("x") ||
            !entry.at("x").is_number_integer() || !entry.contains("y") ||
            !entry.at("y").is_number_integer())
        {
            throw std::runtime_error("Invalid placement in " + path.string());
        }
        layout.placements.push_back(LayoutPlacement{
            entry.at("definition").get<std::string>(), entry.at("x").get<int>(),
            entry.at("y").get<int>()});
    }
    return layout;
}
...
void LayoutRepository::writeJsonFile(const std::filesystem::path& path,
                                     const std::string& contents)
{
    ...
    if (error)
    {
        throw std::runtime_error("Could not create directory for " +
    ...
    const std::filesystem::path tempPath = path.string() + ".tmp";
    {
        ...
        {
            throw std::runtime_error(
                "Could not finish writing temporary file: " +
                tempPath.string());
        }
    }
...
std::filesystem::path
LayoutRepository::sourceMirrorPath(const std::filesystem::path& path) const
{
    if (!m_mirrorToSource || path.is_absolute() ||
        std::string(HEAVENHELL_SOURCE_DIR).empty())
    {
        return {};
    }

    const std::filesystem::path sourcePath =
        std::filesystem::path(HEAVENHELL_SOURCE_DIR) / path;
    std::error_code error;
...
void LayoutRepository::writeJsonFileAndMirror(const std::filesystem::path& path,
...
void LayoutRepository::removeLayoutFileAndMirror(
    const std::filesystem::path& path) const
{
    std::error_code error;
    if (!std::filesystem::remove(path, error) && error)
    {
        throw std::runtime_error("Could not delete layout file " +
                                 path.string() + ": " + error.message());
    }
    ...
    if (!mirrorPath.empty())
    {
        ...
        {
            throw std::runtime_error("Could not delete source layout file " +
                                     mirrorPath.string() + ": " +
                                     error.message());
        }
    }
...
void LayoutRepository::saveLayout(const LayoutInfo& info,
                                  const WorldLayout& layout) const
{
    json data = {{"version", 1}, {"placements", json::array()}};
    for (const LayoutPlacement& placement : layout.placements)
    {
        data["placements"].push_back({{"definition", placement.definition},
                                      {"x", placement.x},
                                      {"y", placement.y}});
    }
...
void LayoutRepository::saveRegistry() const
{
    ...
    {
        data["layouts"].push_back({{"id", info.id},
...
LayoutInfo LayoutRepository::createEmptyLayout()
{
    ...
    LayoutInfo info{id, "Layout " + id.substr(id.find_last_of('_') + 1),
...
void LayoutRepository::deleteLayout(const std::string& id)
{
    ...
    {
        throw std::runtime_error(
            "Choose another active layout before deleting " + id);
    }
    if (m_layouts.size() <= 1)
    {
    ...
    m_layouts.erase(std::remove_if(m_layouts.begin(), m_layouts.end(),
                                   [&id](const LayoutInfo& candidate)
