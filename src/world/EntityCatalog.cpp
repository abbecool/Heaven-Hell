...
json readJsonFile(const std::filesystem::path& path)
{
    ...
    if (!file)
    {
        throw std::runtime_error("Could not load entity definition: " +
                                 path.string());
    }
    json data;
    file >> data;
...
EntityCatalog::EntityCatalog(std::string mobsPath, std::string entitiesPath,
                             std::string itemsPath)
    : m_mobsPath(std::move(mobsPath)), m_entitiesPath(std::move(entitiesPath)),
...
void EntityCatalog::addDefinition(EditorEntityDefinition definition)
{
    ...
    {
        throw std::runtime_error("Duplicate editor entity definition: " +
                                 definition.id);
    }
...
void EntityCatalog::load()
{
    ...
    const auto addJsonDefinitions =
        [this](const std::string& directory, EditorEntityCategory category)
    {
        for (const std::filesystem::path& path : jsonFiles(directory))
        {
            ...
            if (!animation.contains("animation") ||
                !animation.contains("layer"))
            {
                throw std::runtime_error("Visual definition is incomplete: " +
                                         path.string());
            }
            ...
            addDefinition(EditorEntityDefinition{
                id, components.value("CName", id),
                animation.at("animation").get<std::string>(),
                renderLayerFromJson(animation.at("layer")), shadow.scale,
                shadow.offset, category});
        }
    };
    ...
    for (const std::filesystem::path& path : jsonFiles(m_itemsPath))
    {
        ...
        if (!data.contains(id))
        {
            throw std::runtime_error("Invalid item definition: " +
                                     path.string());
        }
        ...
        const CShadow shadow =
            item.contains("shadow") ? CShadow(item.at("shadow")) : CShadow{};
        addDefinition(EditorEntityDefinition{
            id, item.value("name", id), spriteName, RenderLayer::GroundItem,
            shadow.scale, shadow.offset, EditorEntityCategory::Item});
    }
...
EntityCatalog::category(EditorEntityCategory selectedCategory) const
{
