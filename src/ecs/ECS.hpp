...
namespace ecs
{
template<typename... Components> struct Exclude
{
};
} // namespace ecs

template<typename... Components> class ECSView
{
...
public:
    ...
    explicit ECSView(std::vector<const BaseComponentPool*> excludedPools,
                     ComponentPool<Components>*... pools)
        : m_pools(pools...), m_excludedPools(std::move(excludedPools))
    {
        if ((pools && ...))
        {
    ...
    public:
        ...
        auto operator*() const
        {
            ...
            return std::tuple<EntityID, Components&...>{
                entity, std::get<ComponentPool<Components>*>(m_view->m_pools)
                            ->getComponent(entity)...};
        }
    ...
    private:
        void advanceToMatch()
        {
            while (m_view && m_index < m_view->driverSize() &&
                   !m_view->matches(m_view->entityAt(m_index)))
            {
                ++m_index;
            }
        }
...
private:
    ...
    {
        return (std::get<ComponentPool<Components>*>(m_pools)->hasComponent(
                    entity) &&
                ...);
    }
...
template<typename... Components> class ECSConstView
{
    static_assert(sizeof...(Components) > 0,
                  "constView requires at least one component type.");

    using PoolTuple = std::tuple<const ComponentPool<Components>*...>;
...
public:
    ...
    explicit ECSConstView(std::vector<const BaseComponentPool*> excludedPools,
                          const ComponentPool<Components>*... pools)
        : m_pools(pools...), m_excludedPools(std::move(excludedPools))
    {
        if ((pools && ...))
        {
    ...
    {
    public:
        Iterator(const ECSConstView* view, size_t index)
            : m_view(view), m_index(index)
        {
            advanceToMatch();
        }
        ...
        auto operator*() const
        {
            ...
            return std::tuple<EntityID, const Components&...>{
                entity,
                std::get<const ComponentPool<Components>*>(m_view->m_pools)
                    ->getComponent(entity)...};
        }
    ...
    private:
        ...
        const ECSConstView* m_view = nullptr;
        size_t m_index = 0;
    };
...
private:
    void selectDriverPool(const BaseComponentPool* pool)
    {
        const auto& entities = pool->getEntities();
        if (!m_driverEntities || entities.size() < m_driverEntities->size())
        {
    ...
    {
        return (
            std::get<const ComponentPool<Components>*>(m_pools)->hasComponent(
                entity) &&
            ...);
    }
...
private:
    ...
    std::unordered_map<std::type_index, std::unique_ptr<BaseComponentPool>>
        m_componentPools;
    std::vector<EntityID> m_entitiesToRemove;
    std::vector<EntityID> matchingEntities;
    EntityRemovalObserver m_entityRemovalObserver;

    template<typename T> ComponentPool<T>* findComponentPool()
    {
    ...
    template<typename T> const ComponentPool<T>* findComponentPool() const
    {
        std::type_index typeIdx(typeid(T));
        auto it = m_componentPools.find(typeIdx);
        if (it == m_componentPools.end())
        ...
        return reinterpret_cast<const ComponentPool<T>*>(it->second.get());
    }
...
public:
    ...
    void attachChild(EntityID parent, EntityID child, Vec2 relativePos,
    ...
    void queueRemoveEntity(EntityID entity)
    {
        ...
        if (std::find(m_entitiesToRemove.begin(), m_entitiesToRemove.end(),
                      entity) != m_entitiesToRemove.end())
        {
            return;
        }
    ...
    template<typename T> void queueRemoveComponent(EntityID entity)
    {
    ...
    template<typename T> const T& getComponent(EntityID entityId) const
    {
        const ComponentPool<T>* pool = findComponentPool<T>();
        if (pool == nullptr)
        {
    ...
    template<typename T> T& copyComponent(EntityID dst, EntityID src)
    {
    ...
    template<typename T> T& moveComponent(EntityID dst, EntityID src)
    {
        auto& pool = getOrCreateComponentPool<T>();
    ...
    template<typename T> ComponentPool<T>& getOrCreateComponentPool()
    {
        ...
        return *reinterpret_cast<ComponentPool<T>*>(
            m_componentPools[typeIdx].get());
    }

    template<typename... Components> ECSView<Components...> View()
    {
    ...
    ECSView<Components...> View(ecs::Exclude<Excluded...>)
    {
        return ECSView<Components...>(
            std::vector<const BaseComponentPool*>{
                findComponentPool<Excluded>()...},
    ...
    template<typename... Components> const std::vector<EntityID>& ViewEntities()
    {
    ...
    void printEntityComponents(EntityID entity)
    {
        ...
        {
            std::cout << getComponent<CName>(entity).name
                      << "'s components:" << std::endl;
        }
