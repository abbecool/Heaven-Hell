...
void SDLRenderBackend::loadTexture(const std::string& name,
                                   const std::string& path)
{
    ...
    if (!texture)
    {
        throw std::runtime_error("Failed to create texture " + name + " from " +
                                 path + ": " + SDL_GetError());
    }
...
TextureSize SDLRenderBackend::textureSize(const TextureHandle& texture) const
{
    ...
    {
        throw std::runtime_error("Failed to read texture size for " +
                                 texture.name + ": " + SDL_GetError());
    }
    return TextureSize{static_cast<int>(width), static_cast<int>(height)};
}

void SDLRenderBackend::loadFont(const std::string& name,
...
void SDLRenderBackend::drawSprite(const SpriteDrawCommand& command)
{
    ...
    SDL_RenderTextureRotated(m_renderer, texture, &src, &dst, command.angle,
...
{
    drawSprite(SpriteDrawCommand{command.texture, command.src,
                                 m_worldView.worldToScreen(command.dst),
                                 command.angle, command.whiteTint});
}
...
{
    SDL_Color color = {command.color.r, command.color.g, command.color.b,
                       command.color.a};
    SDL_Surface* surface = TTF_RenderText_Blended(
        getFont(command.fontName), command.text.c_str(), 0, color);
    if (!surface)
    {
    ...
    SDL_FRect dst = toSDLRect(command.dst);
    SDL_RenderTextureRotated(m_renderer, texture, nullptr, &dst, 0.0, nullptr,
                             SDL_FLIP_NONE);
    SDL_DestroyTexture(texture);
}
...
{
    drawText(TextDrawCommand{command.text, command.fontName,
                             m_worldView.worldToScreen(command.dst),
                             command.color});
}
