...
OpenGLGlyphAtlas OpenGLGlyphAtlas::build(TTF_Font* font,
                                         const std::string& name)
{
    ...
    {
        OpenGLGlyphInfo info;
        if (!TTF_GetGlyphMetrics(font, static_cast<Uint32>(code), &info.minX,
                                 &info.maxX, &info.minY, &info.maxY,
                                 &info.advance))
        {
            throw std::runtime_error("Failed to read glyph metrics for font " +
                                     name + ": " + SDL_GetError());
        }
        ...
        {
            SDL_Surface* surface =
                TTF_RenderGlyph_Blended(font, static_cast<Uint32>(code), white);
            if (!surface)
            {
                throw std::runtime_error("Failed to render glyph for font " +
                                         name + ": " + SDL_GetError());
            }
            ...
            if (!convertedSurface)
            {
                throw std::runtime_error(
                    "Failed to convert glyph surface for font " + name + ": " +
                    SDL_GetError());
            }
        ...
        pending.glyphs.push_back(
            PendingGlyph{static_cast<char>(code), info, convertedSurface});
    }
    ...
    if (!packGlyphs(atlasSize))
    {
        ...
        {
            throw std::runtime_error(
                "Printable ASCII glyph atlas does not fit for font " + name);
        }
    }

    std::vector<unsigned char> atlasPixels(
        static_cast<size_t>(atlasSize) * static_cast<size_t>(atlasSize) * 4, 0);
    ...
    for (PendingGlyph& glyph : pending.glyphs)
    {
        ...
        {
            throw std::runtime_error("Failed to lock glyph surface for font " +
                                     name + ": " + SDL_GetError());
        }
        ...
        const unsigned char* srcPixels =
            static_cast<const unsigned char*>(glyph.surface->pixels);

        for (int row = 0; row < glyph.info.surfaceHeight; ++row)
        {
            const unsigned char* srcRow =
                srcPixels + row * glyph.surface->pitch;
            unsigned char* dstRow =
                atlasPixels.data() + (static_cast<size_t>(dstY + row) *
                                          static_cast<size_t>(atlasSize) +
                                      static_cast<size_t>(dstX)) *
                                         4;
            std::memcpy(dstRow, srcRow,
                        static_cast<size_t>(glyph.info.surfaceWidth) * 4);
        }
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, atlasSize, atlasSize, 0, GL_RGBA,
