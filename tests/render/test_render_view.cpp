...
void testPointConversion()
{
    ...
    require(nearlyEqual(view.worldToScreenX(100.0f), 320.0f),
            "camera x did not map to origin");
    require(nearlyEqual(view.worldToScreenY(50.0f), 180.0f),
            "camera y did not map to origin");
    require(nearlyEqual(view.worldToScreenX(112.5f), 345.0f),
            "world x conversion was incorrect");
    require(nearlyEqual(view.worldToScreenY(65.0f), 210.0f),
            "world y conversion was incorrect");
}

void testRectConversion()
{
    const RenderView view{10.0f, 20.0f, 1.5f, 100.0f, 80.0f};
    const RectF screenRect =
        view.worldToScreen(RectF{14.0f, 24.0f, 8.0f, 6.0f});
...
void testCameraZoomOrigin()
{
    ...
    const RenderView view{80.0f, 45.0f, totalZoom,
                          static_cast<float>(virtualWidth) * cameraZoom / 2.0f,
                          static_cast<float>(virtualHeight) * cameraZoom /
                              2.0f};

    require(nearlyEqual(view.worldToScreenX(80.0f), 320.0f),
            "legacy zoom origin changed on x");
    require(nearlyEqual(view.worldToScreenY(45.0f), 180.0f),
            "legacy zoom origin changed on y");
}
