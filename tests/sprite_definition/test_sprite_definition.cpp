...
{
    const SpriteDefinition sprite{"walk",
    ...
    require(sprite.texture().name == "character_sheet",
            "texture handle was incorrect");
    ...
    require(nearlyEqual(sprite.frameSize().x, 32.0f),
            "frame width was incorrect");
    require(nearlyEqual(sprite.frameSize().y, 32.0f),
            "frame height was incorrect");
...
{
    const SpriteDefinition sprite{"atlas_walk",
                                  TextureHandle{"atlas"},
                                  6,
                                  2,
                                  2,
    ...
    require(nearlyEqual(sprite.frameSize().x, 32.0f),
            "atlas frame width was incorrect");
    require(nearlyEqual(sprite.frameSize().y, 32.0f),
            "atlas frame height was incorrect");
    requireRect(sprite.frameRect(2, 1), 80.0f, 64.0f, 32.0f, 32.0f);
}

void testMinimumValues()
{
    const SpriteDefinition sprite{"static", TextureHandle{"single"}, 0, 0, 0,
                                  0,        TextureSize{64, 32}};
...
constexpr std::array Tests = {
    TestSupport::TestCase{"texture_size_constructor",
                          testTextureSizeConstructor},
    TestSupport::TestCase{"source_region_constructor",
                          testSourceRegionConstructor},
